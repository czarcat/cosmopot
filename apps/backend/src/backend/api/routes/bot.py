...
async def telegram_webhook(request: Request) -> Response:
    ...
    if runtime is None or runtime.bot is None or runtime.dispatcher is None:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Bot runtime is not initialised")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Invalid update payload") from exc
